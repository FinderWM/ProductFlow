...
def test_inspiration_and_image_session_delete_are_disabled_by_default_and_preserve_records_and_files(
    configured_env: Path,
    db_session,
) -> None:
    ...
    created_inspiration = client.post(
        "/api/inspirations",
        data={"name": "默认禁止删除灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files=[
    ...
    inspiration_payload = created_inspiration.json()
    inspiration_id = inspiration_payload["id"]
    inspiration_root = configured_env / "inspirations" / inspiration_id
    source_asset = next(asset for asset in inspiration_payload["source_assets"] if asset["kind"] == "reference_image")
    ...
    delete_inspiration = client.delete(f"/api/inspirations/{inspiration_id}")
    assert delete_inspiration.status_code == 403
    assert delete_inspiration.json()["detail"] == DELETION_DISABLED_DETAIL
    ...
    db_session.expire_all()
    assert db_session.get(Inspiration, inspiration_id) is not None
