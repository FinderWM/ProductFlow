...
from inspiration_one_backend.infrastructure.queue import recover_unfinished_workflow_runs
...
def test_workflow_run_kickoff_reuses_overlapping_active_node_runs(db_session, configured_env: Path) -> None:
    ...
    first = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id)
    second = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id)
    ...
    with pytest.raises(ValueError, match="灵感产物工作流运行中，稍后删除"):
        delete_inspiration(db_session, inspiration_id=inspiration.id)
...
def test_claim_workflow_node_run_requires_matching_node_id(db_session, configured_env: Path) -> None:
    from inspiration_one_backend.application.inspiration_workflow.run_state import claim_workflow_node_run
    from inspiration_one_backend.application.inspiration_workflows import start_inspiration_workflow_run

    inspiration = create_inspiration(
        db_session,
        name="节点 claim 校验灵感产物",
        category=None,
        price=None,
        source_note=None,
        image_bytes=_make_demo_image_bytes(),
    ...
    kickoff = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id)
...
def test_workflow_run_kickoff_allows_disjoint_active_node_runs(db_session, configured_env: Path) -> None:
    ...
    workflow = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id).workflow
    ...
    first = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id, start_node_id=copy_node.id)
    second = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id, start_node_id=image_node.id)
    duplicate = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id, start_node_id=copy_node.id)
...
def test_workflow_run_endpoint_enqueues_durable_actor_and_reuses_active_run(
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "队列工作流灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("workflow.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]

    first = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={})
    assert first.status_code == 200
    first_run_id = first.json()["runs"][0]["id"]
    assert first.json()["runs"][0]["status"] == "running"
    assert sent_run_ids == [first_run_id]

    second = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={})
    ...
    third = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={})
...
def test_workflow_status_exposes_queue_metadata_and_action_flags(
    ...
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
    submitted = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={})
    assert submitted.status_code == 200
    run_payload = submitted.json()["runs"][0]
    assert run_payload["is_cancelable"] is True
    assert run_payload["is_retryable"] is False

    status = client.get(f"/api/inspirations/{inspiration_id}/workflow/status")
...
def test_workflow_run_enqueue_failure_marks_run_failed(
    ...
    monkeypatch.setattr(
        "inspiration_one_backend.application.inspiration_workflow.execution.enqueue_workflow_run", fail_enqueue
    )
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "入队失败灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("workflow.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]

    response = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={})
...
def test_workflow_run_cancel_marks_active_run_cancelled_and_worker_noops(
    ...
    monkeypatch.setattr(
        "inspiration_one_backend.application.inspiration_workflow.execution._execute_node",
    ...
    cancelled_workflow = cancel_inspiration_workflow_run(db_session, inspiration_id=inspiration.id, run_id=run_id)
    cancelled_payload = serialize_inspiration_workflow(cancelled_workflow).model_dump(mode="json")
    ...
    execute_inspiration_workflow_run(run_id)
    db_session.expire_all()
...
def test_workflow_run_retry_creates_new_run_from_failed_run_without_duplicate_active_run(
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "重试工作流灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("workflow.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
    ...
    sent_run_ids.clear()
    retried = client.post(f"/api/inspirations/{inspiration_id}/workflow/runs/{failed_run_id}/retry")
    ...
    duplicate_retry = client.post(f"/api/inspirations/{inspiration_id}/workflow/runs/{failed_run_id}/retry")
...
def test_workflow_run_retry_rejects_non_retryable_failed_run(
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "不可重试工作流灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("workflow.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
...
def test_recover_unfinished_workflow_runs_requeues_queued_runs(
    ...
    sent_run_ids: list[str] = []
    monkeypatch.setattr(
        "inspiration_one_backend.infrastructure.queue.enqueue_workflow_run",
        lambda run_id: sent_run_ids.append(run_id),
    )
...
def test_inspiration_workflow_worker_defers_queued_run_when_global_running_capacity_full(
    db_session,
    configured_env: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from inspiration_one_backend.application.inspiration_workflow.run_state import (
        INSPIRATION_WORKFLOW_CAPACITY_RETRY_DELAY_MS,
    )
    from inspiration_one_backend.application.inspiration_workflows import (
        cancel_inspiration_workflow_run,
    ...
    occupying_inspiration = create_inspiration(
        db_session,
        name="占用运行容量灵感产物",
        category=None,
        price=None,
    ...
    queued_inspiration = create_inspiration(
        db_session,
        name="等待运行容量灵感产物",
        category=None,
        price=None,
    ...
    assert delayed_requeues == [(dispatched_node_run_id, INSPIRATION_WORKFLOW_CAPACITY_RETRY_DELAY_MS)]

    cancel_inspiration_workflow_run(db_session, inspiration_id=queued_inspiration.id, run_id=queued.run_id)
    execute_inspiration_workflow_node_run(dispatched_node_run_id)
...
def test_duplicate_workflow_messages_noop_for_terminal_or_running_runs(
    ...
    terminal = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id)
    ...
    running = start_inspiration_workflow_run(db_session, inspiration_id=inspiration_two.id)
    ...
    execute_inspiration_workflow_node_run(running_node_run.id)
...
def test_workflow_node_run_message_executes_queued_node_once(
    ...
    monkeypatch.setattr(
        "inspiration_one_backend.application.inspiration_workflow.execution._execute_node", execute_node
    )
    monkeypatch.setattr(
        "inspiration_one_backend.application.inspiration_workflow.execution.enqueue_workflow_run",
        lambda run_id: None,
    ...
    execute_inspiration_workflow_node_run(node_run.id)
    execute_inspiration_workflow_node_run(node_run.id)
...
def test_workflow_node_run_failure_does_not_block_independent_ready_branch(
    ...
    execute_inspiration_workflow_node_run(isolated_node_runs[failing_node.id].id)
    execute_inspiration_workflow_node_run(isolated_node_runs[succeeding_node.id].id)
    execute_inspiration_workflow_run(kickoff.run_id)
...
def test_workflow_retry_requeues_failed_and_blocked_node_runs_only(
    ...
    sent_run_ids: list[str] = []
    retried_workflow = retry_inspiration_workflow_run(
        db_session,
...
def test_workflow_image_generation_timeout_marks_run_node_and_queue_failed(
    ...
    db_session.commit()
    monkeypatch.setattr(
        "inspiration_one_backend.application.inspiration_workflow.image_generation."
    ...
    workflow = run_inspiration_workflow(
        db_session,
        inspiration_id=inspiration.id,
...
def test_workflow_image_generation_provider_failure_uses_safe_reason(
    ...
    from inspiration_one_backend.application.inspiration_workflow.image_generation import (
        WORKFLOW_IMAGE_GENERATION_FAILURE,
    )
    from inspiration_one_backend.application.inspiration_workflow_dependencies import WorkflowExecutionDependencies
    from inspiration_one_backend.application.inspiration_workflows import run_inspiration_workflow
...
def test_workflow_image_generation_provider_failure_exposes_safe_detail(
    ...
    payload = serialize_inspiration_workflow(workflow).model_dump(mode="json")
...
def test_workflow_image_generation_policy_reject_is_not_retryable(
    ...
    workflow_payload = serialize_inspiration_workflow(workflow).model_dump(mode="json")
    ...
    with pytest.raises(BusinessValidationError, match="该工作流节点不可重试"):
        start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id, start_node_id=failed_node_run.node_id)
    ...
    next_config["instruction"] = "改成安全的灵感产物主图提示词"
    ...
    updated_payload = serialize_inspiration_workflow(updated_workflow).model_dump(mode="json")
    ...
    unlocked_kickoff = start_inspiration_workflow_run(
        db_session, inspiration_id=inspiration.id, start_node_id=failed_node.id
    )
...
def test_workflow_node_retry_limit_uses_runtime_config(
    ...
    with pytest.raises(BusinessValidationError, match="该工作流节点不可重试"):
        start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id, start_node_id=failed_node.id)
    ...
    kickoff = start_inspiration_workflow_run(db_session, inspiration_id=inspiration.id, start_node_id=failed_node.id)
...
def test_retry_failed_workflow_nodes_endpoint_starts_all_retryable_failed_nodes(
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "失败节点批量重跑灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    failed_node_ids = {
        node["id"] for node in workflow_payload["nodes"] if node["node_type"] in {"copy_generation", "image_generation"}
    }
    ...
    retried = client.post(f"/api/inspirations/{inspiration_id}/workflow/failed-nodes/retry")
    assert retried.status_code == 202
...
def test_workflow_time_limit_exception_marks_running_node_failed(
    ...
    from inspiration_one_backend.application.inspiration_workflow.run_state import WORKFLOW_WORKER_TIMEOUT_FAILURE
    from inspiration_one_backend.application.inspiration_workflows import (
        execute_inspiration_workflow_run,
        start_inspiration_workflow_run,
    )
    ...
    monkeypatch.setattr(
        "inspiration_one_backend.application.inspiration_workflow.execution._execute_node", raise_time_limit
    )
    _execute_workflow_queue_inline(monkeypatch)
...
def test_workflow_worker_actor_uses_internal_failsafe_time_limit(configured_env: Path) -> None:
    from inspiration_one_backend.workers import (
        ...
        INSPIRATION_WORKFLOW_WORKER_FAILSAFE_TIME_LIMIT_MS,
        get_inspiration_workflow_worker_failsafe_time_limit_ms,
        run_inspiration_workflow_run,
    )

    assert get_inspiration_workflow_worker_failsafe_time_limit_ms() == IMAGE_SESSION_WORKER_FAILSAFE_TIME_LIMIT_MS
    assert INSPIRATION_WORKFLOW_WORKER_FAILSAFE_TIME_LIMIT_MS == IMAGE_SESSION_WORKER_FAILSAFE_TIME_LIMIT_MS
    assert run_inspiration_workflow_run.options["time_limit"] == INSPIRATION_WORKFLOW_WORKER_FAILSAFE_TIME_LIMIT_MS
