...
def _assert_inspiration_context_subset(actual: dict, expected: dict) -> None:
...
def test_inspiration_workflow_dag_runs_and_persists_artifacts(configured_env: Path) -> None:
    ...
    run_payload = _wait_for_workflow_run(client, inspiration_id, status="succeeded")
    ...
    rerun_payload = _wait_for_workflow_run(client, inspiration_id, status="succeeded")
    ...
    inspiration_payload = inspiration_after.json()
    assert any(copy_set["id"] == copy_output["copy_set_id"] for copy_set in inspiration_payload["copy_sets"])
    assert len(inspiration_payload["poster_variants"]) == 4
    reference_assets = [asset for asset in inspiration_payload["source_assets"] if asset["kind"] == "reference_image"]
    ...
    direct_payload = _wait_for_workflow_run(client, inspiration_id, status="failed")
    ...
    try:
        assert session.query(InspirationWorkflow).filter_by(inspiration_id=inspiration_id).count() == 1
    finally:
        session.close()


def test_workflow_success_updates_inspiration_resource_group(configured_env: Path, db_session) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    run = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={})
    assert run.status_code == 200
    completed = _wait_for_workflow_run(client, inspiration_id, status="succeeded")
    node_run_group_ids = {
    ...
    inspiration_after = client.get(f"/api/inspirations/{inspiration_id}")
    assert inspiration_after.status_code == 200
    assert inspiration_after.json()["resource_group_id"] == premium_group.id
    assert inspiration_after.json()["resource_group"]["key"] == "premium-workflow"
...
def test_workflow_run_after_node_excludes_start_node(configured_env: Path) -> None:
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "从节点后运行灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
...
def test_real_image_config_uses_provider_even_when_legacy_poster_mode_is_template(
    ...
    run = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={"start_node_id": image_node["id"]})
    assert run.status_code == 200
    ...
    try:
        posters = list(
            session.scalars(sa.select(PosterVariant).where(PosterVariant.inspiration_id == inspiration_id)).all()
        )
    finally:
        session.close()
...
def test_apply_builtin_scenario_template_appends_real_workflow_nodes_and_edges(
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "场景模板追加灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("scenario-template.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
    original = client.get(f"/api/inspirations/{inspiration_id}/workflow").json()
    ...
    response = client.post(
        f"/api/inspirations/{inspiration_id}/workflow/template-groups",
    ...
    insertable_template_nodes = [
        node for node in template.nodes if node.node_type != WorkflowNodeType.INSPIRATION_CONTEXT
    ]
    ...
    assert (
        len([node for node in workflow["nodes"] if node["node_type"] == WorkflowNodeType.INSPIRATION_CONTEXT.value])
        == 1
    )
    inspiration_context_node = next(node for node in original["nodes"] if node["node_type"] == "inspiration_context")
    ...
    expected_min_x = max(
        480,
        inspiration_context_node["position_x"] + _WORKFLOW_NODE_VISUAL_WIDTH + _WORKFLOW_TEMPLATE_CONTEXT_ANCHOR_GAP,
    )
    expected_min_y = 360
    unmatched_nodes = list(created_nodes)
    persisted_node_ids_by_template_key: dict[str, str] = {"inspiration": inspiration_context_node["id"]}
    ...
    assert any(
        edge["source_node_id"] == inspiration_context_node["id"] and edge["target_node_id"] in created_node_ids
        for edge in created_edges
    )
    db_session.expire_all()
    workflow_row = db_session.query(InspirationWorkflow).filter_by(inspiration_id=inspiration_id, active=True).one()
...
def test_apply_full_canvas_template_reuses_existing_inspiration_context_node(
    configured_env: Path,
    db_session,
) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "复用灵感产物节点灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    assert len([node for node in workflow["nodes"] if node["node_type"] == "inspiration_context"]) == 1
    assert all(node["id"] != inspiration_context_node["id"] for node in created_nodes)

    min_x = min(node.position_x for node in template.nodes if node.node_type != WorkflowNodeType.INSPIRATION_CONTEXT)
    min_y = min(node.position_y for node in template.nodes if node.node_type != WorkflowNodeType.INSPIRATION_CONTEXT)
    min_created_x = min(node["position_x"] for node in created_nodes)
    min_created_y = min(node["position_y"] for node in created_nodes)
    unmatched_nodes = list(created_nodes)
    persisted_node_ids_by_template_key = {"inspiration": inspiration_context_node["id"]}
    for template_node in template.nodes:
        if template_node.node_type == WorkflowNodeType.INSPIRATION_CONTEXT:
            continue
        matched_node = next(
            (
                node
    ...
    assert any(edge["source_node_id"] == inspiration_context_node["id"] for edge in created_edges)
...
def test_builtin_scenario_template_runs_with_auto_inspiration_context_edges(
    configured_env: Path,
    monkeypatch: pytest.MonkeyPatch,
    template_key: str,
) -> None:
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "自动接入测试灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("auto-context.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
    workflow = client.get(f"/api/inspirations/{inspiration_id}/workflow").json()
    inspiration_context_node = next(node for node in workflow["nodes"] if node["node_type"] == "inspiration_context")
    patched_context = client.patch(
        f"/api/workflow-nodes/{inspiration_context_node['id']}",
        json={
            "config_json": {
                "name": "自动接入测试灵感产物",
                "category": "出图工具",
                "price": "199",
                "source_note": "验证场景模板会自动继承灵感产物资料和灵感产物主图。",
    ...
    assert any(
        edge["source_node_id"] == inspiration_context_node["id"] and edge["target_node_id"] == template_copy_node["id"]
        for edge in applied_workflow["edges"]
    )
    assert any(
        edge["source_node_id"] == inspiration_context_node["id"] and edge["target_node_id"] == template_image_node["id"]
        for edge in applied_workflow["edges"]
    )
    ...
    _assert_inspiration_context_subset(
        image_output["context_summary"]["inspiration_context"],
        {
            "name": "自动接入测试灵感产物",
            "category": None,
            "price": None,
    ...
    assert image_output["context_summary"]["inspiration_context"]["dynamic_fields"] == {
        "类目": "出图工具",
        "价格": "199",
    }
    assert image_output["context_summary"]["reference_image_count"] == 1
    ...
    assert provider_input.inspiration_name == "自动接入测试灵感产物"
...
def test_apply_builtin_scenario_template_avoids_existing_node_overlap(configured_env: Path) -> None:
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "场景模板避让灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    assert min(node["position_y"] for node in created_nodes) >= inspiration_context_node["position_y"]
    assert not any(
...
def test_apply_template_requires_existing_active_workflow(configured_env: Path) -> None:
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "未打开画布灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    session = get_session_factory()()
    try:
        assert session.query(InspirationWorkflow).filter_by(inspiration_id=inspiration_id).count() == 0
        assert session.query(InspirationWorkflow).filter_by(inspiration_id=inspiration_id, active=True).count() == 0
    finally:
        session.close()


def test_apply_template_requires_inspiration_context_node(configured_env: Path) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "缺少灵感产物节点灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    assert response.status_code == 400
    assert response.json()["detail"] == "模板需要当前画布中的灵感产物资料节点"
    session = get_session_factory()()
    try:
        workflow_row = session.query(InspirationWorkflow).filter_by(inspiration_id=inspiration_id, active=True).one()
...
def test_apply_template_rejects_unknown_key(configured_env: Path) -> None:
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "模板拒绝灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
...
def test_user_template_group_create_list_rename_archive_and_apply(configured_env: Path) -> None:
    from inspiration_one_backend.infrastructure.db.models import UserCanvasTemplate, WorkflowEdge, WorkflowNode
    from inspiration_one_backend.infrastructure.db.session import get_session_factory
    from inspiration_one_backend.presentation.api import create_app
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "用户模板灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    before_apply = client.get(f"/api/inspirations/{inspiration_id}/workflow").json()
...
def test_user_template_group_preserves_unrun_prompt_config_when_applied(configured_env: Path) -> None:
    from inspiration_one_backend.infrastructure.db.models import UserCanvasTemplate
    from inspiration_one_backend.infrastructure.db.session import get_session_factory
    from inspiration_one_backend.presentation.api import create_app
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "未运行模板灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
...
def test_user_template_group_sanitizes_artifact_config_and_rejects_inspiration_context(configured_env: Path) -> None:
    from inspiration_one_backend.infrastructure.db.models import UserCanvasTemplate, WorkflowNode
    from inspiration_one_backend.infrastructure.db.session import get_session_factory
    from inspiration_one_backend.presentation.api import create_app
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "拒绝模板灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    assert context_save.json()["detail"] == "节点组模板不能包含灵感产物资料节点"
...
def test_user_template_group_ignores_node_outputs_when_saving(configured_env: Path) -> None:
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "输出不入模板灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
...
def test_user_canvas_template_from_workflow_rejects_blank_and_trims_tail_outputs(configured_env: Path) -> None:
    from inspiration_one_backend.infrastructure.db.models import CanvasTemplate, WorkflowNode
    from inspiration_one_backend.infrastructure.db.session import get_session_factory
    from inspiration_one_backend.presentation.api import create_app
    ...
    blank_save = client.post(
        f"/api/inspirations/{blank.json()['id']}/workflow/user-canvas-templates",
    ...
    workflow = client.get(f"/api/inspirations/{tail.json()['id']}/workflow").json()
    tail_node = next(node for node in workflow["nodes"] if node["node_type"] == "tail_splitter")
    created_image = client.post(
        f"/api/inspirations/{tail.json()['id']}/workflow/nodes",
        json={
            "node_type": "image_generation",
            "title": "尾巴后生图",
            "position_x": 640,
    ...
    connected = client.post(
        f"/api/inspirations/{tail.json()['id']}/workflow/edges",
        json={"source_node_id": tail_node["id"], "target_node_id": image_node["id"]},
    )
    assert connected.status_code == 201

    saved = client.post(
        f"/api/inspirations/{tail.json()['id']}/workflow/user-canvas-templates",
        json={
            "title": "尾巴整画布",
    ...
    assert {node["node_type"] for node in summary["preview_nodes"]} == {"inspiration_context", "tail_splitter"}
...
def test_tail_splitter_accepts_provider_source_refs_as_scalar_string(
    ...
    payload = _wait_for_workflow_run(client, inspiration_id, status="waiting_confirmation")
...
def test_admin_can_copy_user_canvas_template_to_global_with_global_category(configured_env: Path) -> None:
    ...
    saved = client.post(
        f"/api/inspirations/{inspiration.json()['id']}/workflow/user-canvas-templates",
...
def test_image_generation_node_normalizes_custom_size_and_rejects_unsafe_dimensions(configured_env: Path) -> None:
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "自定义尺寸灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("inspiration.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
    ...
    updated = client.patch(
        f"/api/workflow-nodes/{image_node['id']}",
        json={"config_json": {"instruction": "生成宽屏灵感产物场景", "size": "3840X2160"}},
    )
    assert updated.status_code == 200
...
def test_inspiration_workflow_singleton_context_and_direct_image_run(configured_env: Path) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    assert duplicate_context.json()["detail"] == "灵感产物资料节点已存在"

    session = get_session_factory()()
    try:
        persisted_workflow = session.scalar(
            sa.select(InspirationWorkflow).where(InspirationWorkflow.inspiration_id == inspiration_id)
        )
    ...
    normalized_response = client.get(f"/api/inspirations/{inspiration_id}/workflow")
    assert normalized_response.status_code == 200
    normalized_workflow = normalized_response.json()
    assert [node["node_type"] for node in normalized_workflow["nodes"]].count("inspiration_context") == 1
    ...
    patched_image = client.patch(
        f"/api/workflow-nodes/{image_node['id']}",
        json={"config_json": {"instruction": "只根据灵感产物资料生成干净主图", "size": "1024x1024"}},
    )
    ...
    assert run_response.status_code == 200
    payload = _wait_for_workflow_run(client, inspiration_id, status="failed")
    assert [node["node_type"] for node in payload["nodes"]] == ["inspiration_context", "image_generation"]
    ...
    assert (
        next(node for node in payload["nodes"] if node["id"] == context_node["id"])["node_type"]
        == "inspiration_context"
    )


def test_direct_downstream_run_uses_latest_saved_inspiration_context(configured_env: Path) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    first_run = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={})
    assert first_run.status_code == 200
    first_payload = _wait_for_workflow_run(client, inspiration_id, status="succeeded")
    ...
    assert image_output["context_summary"]["inspiration_context"]["category"] is None
    assert image_output["context_summary"]["inspiration_context"]["price"] is None
    assert image_output["context_summary"]["inspiration_context"]["dynamic_fields"] == {
        "类目": "户外装备",
        "价格": "249",
    }
    assert (
        image_output["context_summary"]["inspiration_context"]["source_note"]
...
def test_inspiration_context_ignores_unresolved_placeholder_values(
    configured_env: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from inspiration_one_backend.infrastructure.image.base import GeneratedImagePayload
    ...
    assert provider_input.inspiration_name == "测试手机壳"
    assert provider_input.category is None
    assert provider_input.price is None
...
def test_inspiration_context_source_image_reaches_image_generation_context(
    configured_env: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from inspiration_one_backend.infrastructure.image.base import GeneratedImagePayload
    ...
    assert image_output["context_summary"]["reference_image_count"] == 1
    assert any(
        source["label"] == "灵感产物图" and "bag.png" in source["text"] for source in image_output["context_sources"]
    )
...
def test_image_generation_collects_inspiration_context_through_upstream_copy_edge(
    ...
    assert any(
        source["label"] == "灵感产物图" and "chair.png" in source["text"] for source in image_output["context_sources"]
    )
    assert len(captured_inputs) == 1
    provider_input = captured_inputs[0]
    assert provider_input.copy_prompt_mode == "copy"
    assert provider_input.inspiration_name == "折叠露营椅"
    assert provider_input.category is None
    assert provider_input.price is None
...
def test_single_node_workflow_run_reuses_succeeded_upstream_outputs(configured_env: Path) -> None:
    ...
    initial_workflow = client.get(f"/api/inspirations/{inspiration_id}/workflow")
    ...
    inspiration_before = client.get(f"/api/inspirations/{inspiration_id}")
    assert inspiration_before.status_code == 200
    copy_count_before = len(inspiration_before.json()["copy_sets"])
    poster_count_before = len(inspiration_before.json()["poster_variants"])
    source_asset_count_before = len(inspiration_before.json()["source_assets"])
    ...
    single_payload = _wait_for_workflow_run(client, inspiration_id, status="succeeded")
    ...
    inspiration_after = client.get(f"/api/inspirations/{inspiration_id}")
    assert inspiration_after.status_code == 200
    assert len(inspiration_after.json()["copy_sets"]) == copy_count_before
    assert len(inspiration_after.json()["poster_variants"]) == poster_count_before + 1
    assert len(inspiration_after.json()["source_assets"]) == source_asset_count_before + 1
...
def test_single_reference_run_reruns_upstream_when_target_slot_missing_artifact(configured_env: Path) -> None:
    ...
    first_run = client.post(f"/api/inspirations/{inspiration_id}/workflow/run", json={})
    assert first_run.status_code == 200
    assert _wait_for_workflow_run(client, inspiration_id, status="succeeded")["runs"][0]["status"] == "succeeded"
...
def test_image_generation_runs_without_inspiration_context_edge(
    configured_env: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from inspiration_one_backend.infrastructure.image.base import GeneratedImagePayload
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "不会隐式注入的灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    try:
        ...
        session.flush()
        inspiration.current_confirmed_copy_set_id = confirmed_copy.id
        session.commit()
    finally:
        session.close()
    ...
    assert not any(source["label"] == "灵感产物资料" for source in image_output["context_sources"])
    assert len(captured_inputs) == 1
    provider_input = captured_inputs[0]
    assert provider_input.inspiration_name == ""
    assert provider_input.structured_copy_context is None
...
def test_copy_generation_runs_without_inspiration_context_edge(
    configured_env: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from inspiration_one_backend.presentation.api import create_app

    captured_inspirations: list[InspirationInput] = []
    ...
    class CapturingTextProvider:
        ...
        def generate_brief(self, inspiration: InspirationInput) -> tuple[CreativeBriefPayload, str]:
            captured_inspirations.append(inspiration)
            return (
                CreativeBriefPayload(
                    positioning="自由创作定位",
                    audience="自由创作受众",
            ...
            return (
                CopyPayloadV2(
                    ...
                    content=BlocksCopyContent(
                        blocks=[
                            CopyBlock(id="headline", text=f"{inspiration.name} 标题"),
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "不应注入到孤立文案的灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("source.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
    ...
    blank_copy = client.post(
        ...
        json={
            ...
            "position_x": 620,
            "position_y": 420,
            "config_json": {"instruction": "写一句不依赖灵感产物资料的抽象标语"},
        },
    )
    ...
    captured_inspiration = captured_inspirations[0]
    assert captured_inspiration.name == "自由创作"
    ...
    assert not any(source["label"] == "灵感产物资料" for source in copy_output["context_sources"])
...
def test_copy_generation_provider_failure_records_text_usage_stats(
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "失败统计灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("source.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
    ...
    assert selected_run.status_code == 200
    _wait_for_workflow_run(client, inspiration_id, status="failed")
