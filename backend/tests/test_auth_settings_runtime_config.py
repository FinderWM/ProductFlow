...
def test_settings_api_persists_database_overrides(configured_env: Path) -> None:
    ...
    assert initial_items["gallery_entry_tag_max_selection"]["value"] == 10
    assert initial_items["gallery_entry_tag_max_selection"]["category"] == "界面与外观"
    assert initial_items["gallery_entry_tag_max_selection"]["input_type"] == "number"
    assert initial_items["gallery_entry_tag_max_selection"]["minimum"] == 1
    assert initial_items["gallery_entry_tag_max_selection"]["maximum"] == 50
    ...
    updated = client.patch(
        ...
        json={
            "values": {
                ...
                "gallery_entry_tag_max_selection": 12,
    ...
    assert get_runtime_settings().gallery_entry_tag_max_selection == 12
    ...
    try:
        ...
        assert session.get(AppSetting, "gallery_entry_tag_max_selection").value == "12"
...
def test_settings_export_includes_migratable_runtime_config_provider_secrets_and_excludes_env_only(
    ...
    updated = client.patch(
        ...
        json={
            "values": {
                ...
                "gallery_entry_tag_max_selection": 9,
                "gallery_tag_required_on_save": True,
                "deletion_enabled": True,
            }
        },
    ...
    assert payload["runtime_config"]["gallery_entry_tag_max_selection"] == 9
...
def test_settings_import_preview_and_commit_replaces_runtime_and_provider_config(configured_env: Path) -> None:
    ...
    document["runtime_config"]["gallery_entry_tag_max_selection"] = 11
    ...
    assert imported_items["gallery_entry_tag_max_selection"]["value"] == 11
    ...
    try:
        ...
        assert session.get(AppSetting, "gallery_entry_tag_max_selection").value == "11"
        assert session.get(AppSetting, "gallery_tag_required_on_save").value == "true"
...
def test_image_generation_max_dimension_runtime_config_controls_size_bounds(configured_env: Path) -> None:
    ...
    assert runtime.json() == {
        ...
        "gallery_entry_tag_max_selection": 10,
