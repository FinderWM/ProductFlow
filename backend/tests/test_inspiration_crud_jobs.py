...
def test_inspiration_create_persists_source_note_for_ai_context(configured_env: Path) -> None:
    ...
    minimal = client.post(
        "/api/inspirations",
        data={"name": "极简灵感产物壳", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
...
def test_default_inspiration_create_preserves_lazy_workflow_behavior(configured_env: Path, db_session) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "默认画布灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    assert db_session.query(InspirationWorkflow).filter_by(inspiration_id=inspiration_id).count() == 0
    ...
    db_session.expire_all()
    assert db_session.query(InspirationWorkflow).filter_by(inspiration_id=default_key.json()["id"]).count() == 0


def test_inspiration_create_materializes_full_canvas_template(configured_env: Path, db_session) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    inspiration_id = created.json()["id"]
    db_session.expire_all()
    workflow = db_session.query(InspirationWorkflow).filter_by(inspiration_id=inspiration_id, active=True).one()
        ...
        for node in unmatched_nodes:
            ...
            if template_node.node_type == WorkflowNodeType.INSPIRATION_CONTEXT:
                if all(node.config_json.get(key) == value for key, value in expected_config.items()):
                    assert node.config_json["name"] == "模板画布灵感产物"
                    assert node.config_json["owner_id"] == inspiration_id
...
def test_inspiration_create_requires_resource_group_for_authorized_entry(db_session, configured_env: Path) -> None:  # noqa: ARG001
    from inspiration_one_backend.presentation.api import create_app
...
def test_inspiration_create_rejects_ungranted_resource_group_for_member(configured_env: Path) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    created_group = admin_client.post(
        ...
        json={"key": "premium-inspiration", "name": "高阶灵感分组", "sort_order": 20, "enabled": True},
    )
    assert created_group.status_code == 200
    premium_group_id = created_group.json()["id"]
    created_user = admin_client.post(
        "/api/rbac/users",
        json={"username": "inspiration-member", "display_name": "Inspiration Member"},
    )
    ...
    set_password = user_client.post(
        "/api/auth/password",
        json={
            "username": "inspiration-member",
            "password": "inspiration-member-password",
...
def test_inspiration_create_filters_canvas_template_by_entry_mode(configured_env: Path, db_session) -> None:
    ...
    db_session.expire_all()
    workflow = db_session.query(InspirationWorkflow).filter_by(inspiration_id=accepted.json()["id"], active=True).one()
    assert workflow.initial_entry_mode == "copy"


def test_inspiration_create_template_persists_text_entry_context_for_summary(configured_env: Path, db_session) -> None:
    from inspiration_one_backend.application.inspiration_workflow.user_templates import create_global_canvas_template
    from inspiration_one_backend.presentation.api import create_app
    ...
    db_session.expire_all()
    workflow = db_session.query(InspirationWorkflow).filter_by(inspiration_id=created.json()["id"], active=True).one()
    context_node = (
        db_session.query(WorkflowNode)
        .filter_by(workflow_id=workflow.id, node_type=WorkflowNodeType.INSPIRATION_CONTEXT)
        .one()
    )
    inspiration_id = created.json()["id"]
...
def test_inspiration_list_filters_by_selected_resource_group(configured_env: Path) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    assert created_group.status_code == 200
    premium_group_id = created_group.json()["id"]
    default_inspiration = client.post(
        "/api/inspirations",
    ...
    assert default_inspiration.status_code == 201
    premium_inspiration = client.post(
        "/api/inspirations",
        data={"name": "高阶分组灵感", "resource_group_id": premium_group_id},
        files={"image": ("premium-list.png", _make_demo_image_bytes(), "image/png")},
    )
    assert premium_inspiration.status_code == 201

    default_list = client.get("/api/inspirations", params={"resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID})
    assert default_list.status_code == 200
    assert {item["id"] for item in default_list.json()["items"]} == {default_inspiration.json()["id"]}
    assert default_list.json()["items"][0]["resource_group"]["key"] == "default"

    premium_list = client.get("/api/inspirations", params={"resource_group_id": premium_group_id})
...
def test_successful_workflow_generation_updates_inspiration_resource_group(
    ...
    created_inspiration = client.post(
        "/api/inspirations",
    ...
    assert created_inspiration.json()["resource_group_id"] == DEFAULT_GENERATION_RESOURCE_GROUP_ID
    ...
    run_response = client.post(
        f"/api/inspirations/{inspiration_id}/workflow/run", json={"start_node_id": copy_node["id"]}
    )
    assert run_response.status_code == 200
    _wait_for_workflow_run(client, inspiration_id, status="succeeded")

    db_session.expire_all()
    inspiration = db_session.get(Inspiration, inspiration_id)
    assert inspiration is not None
    assert inspiration.resource_group_id == premium_group_id

    detail = client.get(f"/api/inspirations/{inspiration_id}")
    assert detail.status_code == 200
    assert detail.json()["resource_group"]["key"] == "premium-workflow"


def test_inspiration_create_accepts_broad_builtin_canvas_template_key(configured_env: Path, db_session) -> None:
    from inspiration_one_backend.presentation.api import create_app
...
def test_inspiration_list_summary_uses_latest_run_first_generated_image(configured_env: Path, db_session) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    workflow = db_session.query(InspirationWorkflow).filter_by(inspiration_id=inspiration.id, active=True).one()
    context_node = (
        db_session.query(WorkflowNode)
        .filter_by(workflow_id=workflow.id, node_type=WorkflowNodeType.INSPIRATION_CONTEXT)
        .one()
    ...
    old_poster = PosterVariant(
        ...
        template_name="old",
        storage_path="inspirations/posters/old.png",
        width=800,
        height=800,
        created_at=now,
    )
    first_poster = PosterVariant(
        ...
        kind=PosterKind.MAIN_IMAGE,
        template_name="first",
        storage_path="inspirations/posters/first.png",
        width=800,
        height=800,
    ...
    latest_created_poster = PosterVariant(
        ...
        storage_path="inspirations/posters/latest-created.png",
        width=800,
        height=800,
    ...
    listed = client.get("/api/inspirations", params={"resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID})
    assert listed.status_code == 200
    item = next(item for item in listed.json()["items"] if item["id"] == inspiration.id)
...
def test_legacy_jobrun_routes_are_removed(configured_env: Path) -> None:
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "无传统任务灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("legacy.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]

    assert client.post(f"/api/inspirations/{inspiration_id}/copy-jobs").status_code == 404
    assert client.post(f"/api/inspirations/{inspiration_id}/poster-jobs").status_code == 404
    assert client.post("/api/posters/missing/regenerate").status_code == 404
    assert client.get("/api/jobs/missing").status_code == 404

    history = client.get(f"/api/inspirations/{inspiration_id}/history")
    assert history.status_code == 200
    assert set(history.json()) == {"copy_sets", "poster_variants"}


def test_inspiration_history_can_filter_by_resource_group(configured_env: Path, db_session) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "分组历史灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
    ...
    db_session.add_all(
        [
            PosterVariant(
                ...
                storage_path="inspirations/history/legacy.png",
                width=800,
                height=800,
                resource_group_id=None,
            ),
            PosterVariant(
                ...
                storage_path="inspirations/history/default.png",
                width=800,
                height=800,
                resource_group_id=DEFAULT_GENERATION_RESOURCE_GROUP_ID,
            ),
            PosterVariant(
                ...
                storage_path="inspirations/history/premium.png",
                width=800,
                height=800,
                resource_group_id=premium_group.id,
            ),
    ...
    default_history = client.get(
        f"/api/inspirations/{inspiration_id}/history",
...
def test_inspiration_can_be_deleted_from_api(configured_env: Path, db_session) -> None:
    from inspiration_one_backend.presentation.api import create_app
    ...
    created = client.post(
        "/api/inspirations",
        data={"name": "待删除灵感产物", "resource_group_id": DEFAULT_GENERATION_RESOURCE_GROUP_ID},
        files={"image": ("delete.png", _make_demo_image_bytes(), "image/png")},
    )
    assert created.status_code == 201
    inspiration_id = created.json()["id"]
    inspiration_root = configured_env / "inspirations" / inspiration_id
    assert inspiration_root.exists()
    ...
    inspiration_with_artifacts = client.get(f"/api/inspirations/{inspiration_id}")
    assert inspiration_with_artifacts.status_code == 200
    assert inspiration_with_artifacts.json()["copy_sets"]
    assert inspiration_with_artifacts.json()["poster_variants"]

    _enable_deletion(client)
    deleted = client.delete(f"/api/inspirations/{inspiration_id}")
    ...
    db_session.expire_all()
    persisted = db_session.get(Inspiration, inspiration_id)
    assert persisted is not None
    assert persisted.deleted_at is not None
    assert inspiration_root.exists()


def test_reference_images_can_be_attached_to_inspiration(db_session, configured_env: Path) -> None:
    inspiration = create_inspiration(
        db_session,
        name="陶瓷马克杯",
        category="家居",
...
def test_inspiration_status_filter_uses_database_pagination_before_eager_loading(
    db_session, configured_env: Path
) -> None:
    draft = create_inspiration(
        db_session,
        name="草稿灵感产物",
        category=None,
        price=None,
    ...
    poster_ready = create_inspiration(
        db_session,
        name="海报灵感产物",
        category=None,
        price=None,
    ...
    poster_copy_set = CopySet(
        inspiration_id=poster_ready.id,
        status=CopyStatus.CONFIRMED,
        structured_payload={
            "version": 2,
            "summary": "海报标题",
    ...
    db_session.add(
        PosterVariant(
            ...
            kind=PosterKind.MAIN_IMAGE,
            template_name="test",
            storage_path="inspirations/poster/poster.png",
            width=800,
            height=800,
        )
    )
    ...
    inspiration_selects: list[str] = []

    @event.listens_for(db_session.bind, "before_cursor_execute")
    def record_inspiration_query(conn, cursor, statement, parameters, context, executemany):
        normalized_statement = " ".join(statement.lower().split())
        if (
            normalized_statement.startswith("select inspirations.id")
            and "from inspirations" in normalized_statement
            and "limit" in normalized_statement
        ):
            inspiration_selects.append(normalized_statement)

    try:
        inspirations, total = list_inspirations(
            db_session,
            status=InspirationWorkflowState.DRAFT,
            page=1,
            page_size=1,
            resource_group_id=DEFAULT_GENERATION_RESOURCE_GROUP_ID,
        )
    finally:
        event.remove(db_session.bind, "before_cursor_execute", record_inspiration_query)

    assert total == 1
    assert [inspiration.id for inspiration in inspirations] == [draft.id]
    assert len(inspiration_selects) == 1
    assert "exists" in inspiration_selects[0]
    assert "limit" in inspiration_selects[0]

    copy_inspirations, copy_total = list_inspirations(
        db_session,
        status=InspirationWorkflowState.COPY_READY,
        page=1,
        page_size=10,
        resource_group_id=DEFAULT_GENERATION_RESOURCE_GROUP_ID,
    )
    poster_inspirations, poster_total = list_inspirations(
        db_session,
        status=InspirationWorkflowState.POSTER_READY,
        page=1,
        page_size=10,
        resource_group_id=DEFAULT_GENERATION_RESOURCE_GROUP_ID,
    )
    failed_inspirations, failed_total = list_inspirations(
        db_session,
        status=InspirationWorkflowState.FAILED,
        page=1,
        page_size=10,
        resource_group_id=DEFAULT_GENERATION_RESOURCE_GROUP_ID,
    )
    ...
    assert copy_total == 1
    assert [inspiration.id for inspiration in copy_inspirations] == [copy_ready.id]
    assert poster_total == 1
    assert [inspiration.id for inspiration in poster_inspirations] == [poster_ready.id]
...
def test_inspiration_reference_image_can_be_deleted(configured_env: Path, db_session) -> None:
    ...
    assert rejected.status_code == 400
    assert "只能删除灵感产物参考图" in rejected.json()["detail"]
