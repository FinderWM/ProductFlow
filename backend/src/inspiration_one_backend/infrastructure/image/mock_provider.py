...
from inspiration_one_backend.infrastructure.image.base import GeneratedImagePayload, ImageProvider, parse_size
...
class MockImageProvider(ImageProvider):
    ...
    def generate_poster_image(
        ...
        headline = context_lines[0].removeprefix("摘要：") if context_lines else poster.inspiration_name
        points = context_lines[1:4]

        draw.text((84, 90), headline[:28], font=title_font, fill=foreground)
        draw.text((84, 180), poster.inspiration_name[:32], font=body_font, fill=foreground)
