...
from inspiration_one_backend.config import Settings, get_settings
from inspiration_one_backend.infrastructure.db.models import utcnow

_INSPIRATION_ONE_FILE_HANDLER = "_inspiration_one_file_handler"
_INSPIRATION_ONE_STREAM_HANDLER = "_inspiration_one_stream_handler"
...
def get_log_file_path(settings: Settings | None = None) -> Path:
    """Return the effective persistent Inspiration One log file path."""

    settings = settings or get_settings()
    return settings.log_dir.expanduser() / "inspiration-one.log"
...
def configure_logging(settings: Settings | None = None) -> None:
    ...
    propagate to the root logger, so Inspiration One mirrors them to the same rotating file handler while leaving their
    ...
    if not stream_handler_exists:
        ...
        setattr(stream_handler, _INSPIRATION_ONE_STREAM_HANDLER, True)
...
class _InspirationOneFormatter(logging.Formatter):
...
def _ensure_shared_file_handler(
    ...
    """Install one shared Inspiration One file handler for all persistent log mirrors."""
    ...
    if matching_handler is None:
        ...
        setattr(matching_handler, _INSPIRATION_ONE_FILE_HANDLER, True)

    matching_handler.setFormatter(formatter)
    matching_handler.setLevel(level)
    for configured_logger in _configured_file_loggers():
        _remove_inspiration_one_file_handlers(configured_logger, keep=matching_handler)
...
def _mirror_uvicorn_logs_to_file(
    ...
    for logger_name in _UVICORN_FILE_LOGGERS:
        ...
        if _logger_records_reach_root(logger):
            _remove_inspiration_one_file_handlers(logger, keep=None)
            continue
        logger.setLevel(level)
        _remove_inspiration_one_file_handlers(logger, keep=file_handler)
...
def _find_matching_file_handler(resolved_log_file: Path) -> RotatingFileHandler | None:
    for logger in _configured_file_loggers():
        for handler in logger.handlers:
            if not getattr(handler, _INSPIRATION_ONE_FILE_HANDLER, False):
                continue
...
def _remove_inspiration_one_file_handlers(
    logger: logging.Logger,
    *,
    keep: RotatingFileHandler | None,
) -> None:
    for handler in list(logger.handlers):
        if handler is keep or not getattr(handler, _INSPIRATION_ONE_FILE_HANDLER, False):
            continue
