...
from inspiration_one_backend.domain.enums import WorkflowRunStatus
...
def remove_pending_tail_confirmation(run: WorkflowRun, *, tail_node_id: str, plan_id: str) -> bool:
    ...
    next_items = [item for item in current if item["tail_node_id"] != tail_node_id or item["plan_id"] != plan_id]
