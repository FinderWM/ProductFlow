...
from inspiration_one_backend.application.ownership import (
...
INSPIRATION_CONTEXT_TEMPLATE_ASSET_CONFIG_KEYS = frozenset(
    {
...
INSPIRATION_CONTEXT_TEMPLATE_RUNTIME_CONFIG_KEYS = frozenset({"owner_id", "entry_type", "category", "price"})
...
def canvas_template_tables_available() -> bool:
    ...
    return all(inspector.has_table(table_name) for table_name in ("canvas_templates", "canvas_template_categories"))
...
def _builtin_template_payload(template: CanvasTemplate, category: CanvasTemplateCategory) -> dict[str, Any]:
    ...
    return _sanitize_template_payload_inspiration_context_configs(payload)
...
def extract_reusable_node_config(node: WorkflowNode, *, retain_prompt_text: bool = True) -> dict[str, Any]:
    if node.node_type == WorkflowNodeType.INSPIRATION_CONTEXT:
        reusable_config = _sanitize_inspiration_context_template_config(node.config_json or {})
    else:
...
def _sanitize_inspiration_context_template_config(config_json: dict[str, Any]) -> dict[str, Any]:
    sanitized = _sanitize_reusable_config(
        {
            key: value
            for key, value in normalize_inspiration_context_config(config_json).items()
            if key not in INSPIRATION_CONTEXT_TEMPLATE_ASSET_CONFIG_KEYS
            and key not in INSPIRATION_CONTEXT_TEMPLATE_RUNTIME_CONFIG_KEYS
        },
...
def _sanitize_template_payload_inspiration_context_configs(payload: dict[str, Any]) -> dict[str, Any]:
    nodes = payload.get("nodes")
    if not isinstance(nodes, list):
        return payload
    for node in nodes:
        if not isinstance(node, dict) or node.get("node_type") != WorkflowNodeType.INSPIRATION_CONTEXT.value:
            continue
        config_json = node.get("config_json")
        node["config_json"] = _sanitize_inspiration_context_template_config(
