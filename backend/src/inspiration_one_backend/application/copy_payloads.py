...
from inspiration_one_backend.infrastructure.db.models import CopySet
...
def copy_payload_context_text(payload: CopyPayloadV2) -> str:
    ...
    if isinstance(payload.content, FreeformCopyContent):
        ...
        for section in payload.content.sections:
            ...
            item_text = "；".join(f"{item.label or item.role or '条目'}：{item.text}" for item in section.items)
