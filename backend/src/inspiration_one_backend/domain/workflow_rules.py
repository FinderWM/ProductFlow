...
def should_execute_missing_upstream(source_node: WorkflowRuleNode, target_node: WorkflowRuleNode) -> bool:
    ...
    if source_node.node_type in {WorkflowNodeType.IMAGE_GENERATION, WorkflowNodeType.IMAGE_ENHANCE}:
        return target_node.node_type in {
    ...
    if source_node.node_type == WorkflowNodeType.TAIL_SPLITTER:
        return target_node.node_type in {WorkflowNodeType.IMAGE_GENERATION, WorkflowNodeType.IMAGE_ENHANCE}
    return False
    ...
    *,
    ...
    for node_id in ordered_ids:
        if node_id not in run_node_id_set or node_id not in queued_node_id_set:
            continue
        if all(
            source_id not in run_node_id_set or source_id in succeeded_node_id_set for source_id in incoming[node_id]
        ):
