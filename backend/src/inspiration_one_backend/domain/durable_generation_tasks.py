...
class DurableGenerationTaskContract:
    ...
    The contract intentionally describes existing business models instead of replacing them.
    Inspiration workflow runs keep their run/node-run split, while continuous image-session
    tasks keep their task-level queued/running state.
    """
...
WORKFLOW_RUN_GENERATION_TASK_CONTRACT = DurableGenerationTaskContract(
    name="inspiration_workflow_run",
    durable_model_name="WorkflowRun",
    actor_name="run_inspiration_workflow_run",
    ...
    status_snapshot_source="InspirationWorkflowStatusSnapshot",
