...
from inspiration_one_backend.application.inspiration_workflows import latest_workflow_runs
from inspiration_one_backend.application.moderation import moderation_state_for_resource
...
class InspirationWorkflowResponse(BaseModel):
    id: str
    inspiration_id: str
    title: str
    active: bool
...
class InspirationWorkflowStatusResponse(BaseModel):
    id: str
    inspiration_id: str
    title: str
    active: bool
...
def serialize_user_canvas_template_summary(template: DbCanvasTemplate) -> CanvasTemplateSummaryResponse:
    from inspiration_one_backend.application.inspiration_workflow.user_templates import (
        canvas_template_row_to_canvas_template,
    )

    return serialize_canvas_template_summary(canvas_template_row_to_canvas_template(template))


def serialize_inspiration_workflow_status(
    snapshot: InspirationWorkflowStatusSnapshot,
) -> InspirationWorkflowStatusResponse:
    workflow = snapshot.workflow
    nodes = snapshot.nodes
    return InspirationWorkflowStatusResponse(
        id=workflow.id,
        inspiration_id=workflow.inspiration_id,
        title=workflow.title,
        active=workflow.active,
