...
def require_product_usable(product: Product) -> None:
    require_resource_usable("product", product)
...
def resource_type_for_instance(resource: Any) -> ResourceType:
    if isinstance(resource, Product):
        return "product"
...
def effective_disabled_source_for(resource_type: ResourceType, resource: Any) -> DisabledSource | None:
    if resource_type == "product":
        return _own_disabled_source("product", resource)
    if resource_type == "source_asset":
        return _own_disabled_source("source_asset", resource) or _product_disabled_source(resource.product)
    if resource_type == "poster_variant":
        return _own_disabled_source("poster_variant", resource) or _product_disabled_source(resource.product)
    if resource_type == "image_session":
        return _own_disabled_source("image_session", resource) or _product_disabled_source(resource.product)
...
def _product_disabled_source(product: Product | None) -> DisabledSource | None:
    return _own_disabled_source("product", product) if product is not None else None


def _image_session_disabled_source(image_session: ImageSession | None) -> DisabledSource | None:
    if image_session is None:
        return None
    return _own_disabled_source("image_session", image_session) or _product_disabled_source(image_session.product)
...
def _load_resource_for_moderation(session: Session, resource_type: ResourceType, resource_id: str) -> Any | None:
    if resource_type == "product":
        return session.scalar(
            select(Product)
            .options(selectinload(Product.owner), selectinload(Product.disabled_by))
            .where(Product.id == resource_id)
        )
    if resource_type == "source_asset":
        return session.scalar(
            select(SourceAsset)
            .options(
                selectinload(SourceAsset.product).selectinload(Product.owner),
                selectinload(SourceAsset.product).selectinload(Product.disabled_by),
    ...
    if resource_type == "poster_variant":
        return session.scalar(
            ...
            .options(
                selectinload(PosterVariant.product).selectinload(Product.owner),
                selectinload(PosterVariant.product).selectinload(Product.disabled_by),
    ...
    if resource_type == "image_session":
        return session.scalar(
            ...
            .options(
                ...
                selectinload(ImageSession.product).selectinload(Product.disabled_by),
            )
    ...
    if resource_type == "image_session_asset":
        return session.scalar(
            ...
            .options(
                ...
                selectinload(ImageSessionAsset.session).selectinload(ImageSession.product).selectinload(
                    Product.disabled_by
                ),
            )
    ...
    if resource_type == "image_gallery_entry":
        return session.scalar(
            ...
            .options(
                ...
                .selectinload(Product.disabled_by),
            )
...
def _resource_owner(resource_type: ResourceType, resource: Any) -> Any | None:
    if resource_type in {"product", "image_session", "image_session_asset", "image_gallery_entry"}:
        return getattr(resource, "owner", None)
    if resource_type in {"source_asset", "poster_variant"}:
        product = getattr(resource, "product", None)
        return getattr(product, "owner", None) if product is not None else None
