"""add inspiration workflow dag tables"""
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(["workflow_id"], ["inspiration_workflows.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
...
def downgrade() -> None:
    ...
    op.drop_index("uq_inspiration_workflows_one_active_per_inspiration", table_name="inspiration_workflows")
    op.drop_table("inspiration_workflows")
