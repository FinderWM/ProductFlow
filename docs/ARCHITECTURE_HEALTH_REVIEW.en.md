# Inspiration One Architecture Health Review
...
- Iterative image generation and inspiration workflows both use lightweight status polling while running, then refresh full details after completion.
...
Backend code lives under `backend/src/inspiration_one_backend/`:

- `presentation/`: FastAPI app, routes, schemas, auth dependencies, upload validation, and error mapping.
- `application/`: inspirations, traditional copy/poster jobs, iterative image generation, gallery, generation admission, and inspiration workflow use cases.
...
The InspirationWorkflow application is no longer a single file carrying all responsibilities. Current modules include:

- `inspiration_workflows.py`: stable facade for routes.
- `inspiration_workflow/graph.py`: workflow queries, default graph structure, and lightweight status snapshot.
- `inspiration_workflow/mutations.py`: nodes, edges, reference image slots, and copy-node editing.
- `inspiration_workflow/query.py`: inspiration workflow detail query.
- `inspiration_workflow/execution.py`: run creation, node scheduling, and non-image node execution.
- `inspiration_workflow/run_state.py`: node-run claim, failure, cancellation, and capacity requeue state transitions.
- `inspiration_workflow/image_generation.py`: image-generation node execution, provider timeout, safe failure, and artifact writeback.
- `inspiration_workflow/context.py`: upstream context collection and provider input context construction.
- `inspiration_workflow/artifacts.py`: `CopySet`, `SourceAsset`, `PosterVariant`, and related artifact writeback helpers.
- `inspiration_workflow/templates.py`: canvas template materialization helpers.
- `inspiration_workflow/user_templates.py`: user-saved node-group template create/list/rename/archive/apply use cases.
- `inspiration_workflow_dependencies.py`: execution dependency injection seam for tests and future orchestration of text/image providers and renderer.

There are currently two background execution state families:

- `WorkflowRun` / `WorkflowNodeRun`: inspiration DAG workflow runs.
...
- `App.tsx`: route entrypoint, including `/login`, `/inspirations`, `/inspirations/new`, `/inspirations/:inspirationId`, `/image-chat`, `/inspirations/:inspirationId/image-chat`, `/gallery`, `/help`, and `/settings`.
- `pages/`: login, inspiration list, inspiration creation, inspiration detail, iterative image generation, gallery, help, and settings pages.
- `pages/inspiration-detail/`: inspiration detail workbench page-local components, canvas helpers, download helpers, tests, and types.
...
### 3.1 Documentation Aligned with Inspiration Reality
...
- Inspiration One workbench, iterative image generation, gallery, settings page, and runtime configuration.
...
### 3.2 Inspiration Workflow Split

The inspiration DAG workflow has been split from a concentrated application file into responsibility-named modules. Routes still call through a stable facade, reducing the API-layer impact of the split.
...
- `web/src/pages/inspiration-detail/galleryImages.test.ts`
- `web/src/pages/inspiration-detail/reactFlowAdapters.test.ts`
- `web/src/pages/inspiration-detail/selection.test.ts`
- `web/src/pages/inspiration-detail/utils.test.ts`
...
- `ImageGalleryEntry` keeps source session, round, linked inspiration, prompt, size, model, and download entrypoint.
...
- Inspiration workflows poll `/api/inspirations/{inspiration_id}/workflow/status` while running.
...
This reduces repeated serialization of large objects, image-history rerenders, and accidental overwrites of local interaction state while inspiration detail and iterative image pages are running.

## 4. Current Main Risks

### R1. Application Still Carries Many Domain Rules

InspirationWorkflow has been split, but business rules still mostly live in application modules around SQLAlchemy models. This is acceptable short-term. If node-level retry, skip, duplicate, version comparison, and provider routing keep expanding, rules may spread further across execution, mutations, context, and artifacts.
...
Vitest now covers helpers and some hooks, but real InspirationDetail workbench interactions still rely on manual verification: node dragging, edge creation/deletion, right-panel switching, save-draft-before-run, image fill, and download misclick protection.
...
The Docker Compose self-hosted path is available, but Inspiration One is still not a full production platform:
...
1. **Prioritize key InspirationDetail workbench interaction tests**
...
4. **Keep splitting by real hotspots, not global rewrites**  
   The InspirationWorkflow and InspirationDetail split direction has worked. Future splits should follow real modification hotspots from new features, avoiding repository, domain service, or complex frontend state layers introduced only for architecture completeness.
