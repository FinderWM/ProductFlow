...
Inspiration One 是一个开源、自托管的灵感产物素材工作台，面向单人商家、小团队运营者或希望在自己基础设施里管理 AI 素材链路的开发者。

它不是托管 SaaS、不是多租户开放平台，也不承诺替代人工运营判断。当前核心目标是：

> 把一个灵感产物从输入素材推进到可编辑文案、可下载海报、可复用图片素材、可收藏生成图和可追踪工作流状态。

## 2. 目标用户

- 需要快速制作灵感产物标题、卖点、主图和促销海报的商家。
...
1. 使用管理员密钥登录。
2. 创建灵感产物，上传灵感产物原图、填写灵感产物名，并选择空白画布或电商场景模板。
3. 进入灵感产物工作台，补充类目、价格、灵感产物说明和生成方向。
4. 通过文案节点生成并编辑结构化文案；后续生图直接读取结构化文案上下文。
5. 通过生图节点生成图片，并把结果填充到下游参考图槽位。
6. 下载图片/海报，或在右侧图库面板回看灵感产物素材历史。
7. 移动端灵感产物列表使用卡片和浮动分页；灵感产物详情保留画布为主视图，通过底部工具栏打开运行、单节点、模板、详情、日志和图库。
...
6. 将满意的生成图挂回某个灵感产物作为参考素材、保存为灵感产物主图参考，或收藏到画廊。
7. 移动端使用主视图、抽屉和底部面板组合：顶部栏提供会话抽屉、当前会话标题/重命名和历史抽屉；主视图保留状态、当前结果和供应商提示；左侧抽屉管理会话；右侧窄抽屉选择分支/候选历史；底部生成面板承载灵感产物关联、参考图、提示词、尺寸、候选数量和高级图片工具参数。

### 3.3 灵感产物 DAG 工作流

1. 在灵感产物详情中打开工作流工作台。
2. 创建或调整节点：灵感产物上下文、参考图、文案生成、图片生成。
3. 使用内置场景模板追加常用流程，或多选节点保存为用户模板。
4. 连接节点形成 DAG。
5. 启动后台工作流运行，必要时取消运行或重试失败运行。
6. 在数据库中持久化运行、节点状态和失败原因。
7. 运行中前端轮询轻量工作流状态；结束后刷新完整工作流、灵感产物详情和历史产物。
...
3. 画廊条目保留来源会话、关联灵感产物、提示词、尺寸、模型和下载入口。

### 3.5 产品内帮助

1. 从顶部导航进入 `/help`。
2. 查看快速开始、工作台、模板、运行状态、支持边界和常见问题。
3. 从帮助页回到灵感产物工作台或文/图生图继续操作。

## 4. 核心对象

- `Inspiration`：灵感产物实体，包含名称、类目、价格和输入素材。
- `SourceAsset`：灵感产物素材，包含原始主图、参考图、处理后灵感产物图等类型。
- `CreativeBrief`：系统生成的灵感产物理解结果，为文案和海报提供统一语义。
...
- `InspirationWorkflow` / `WorkflowNode` / `WorkflowEdge` / `WorkflowRun`：灵感产物 DAG 工作流结构和运行记录。
...
- `/login`：管理员密钥登录。
- `/inspirations`：灵感产物列表。
- `/inspirations/new`：创建灵感产物。
- `/inspirations/:inspirationId`：灵感产物详情、文案/海报主链路、历史、DAG 工作流。
- `/gallery`：生成图画廊。
- `/help`：产品内帮助页。
- `/settings`：provider、模型、上传限制、任务重试等业务配置。
- `/image-chat` 和 `/inspirations/:inspirationId/image-chat`：连续生图与素材挂回灵感产物。

## 6. V1 已实现验收面

对单个自托管部署，当前版本应能完成：

1. 用本地 `mock` provider 在无外部模型密钥时跑通灵感产物链路。
2. 用 PostgreSQL 保存灵感产物、素材、文案、海报、任务、图片会话和工作流状态。
3. 用 Redis + Dramatiq 执行异步文案/海报任务和灵感产物工作流。
4. 用 durable `ImageSessionGenerationTask` 执行连续生图，支持队列位置、失败原因和完成后刷新。
5. 使用完整场景模板创建灵感产物，并在工作台中插入同一套内置场景模板或用户保存的节点组模板。
6. 在前端查看任务状态、工作流节点状态、生成队列概览、失败原因和历史记录。
7. 在运行中使用轻量 status 接口刷新任务/工作流状态，避免高频拉取完整大对象。
8. 对可恢复的连续生图任务和灵感产物工作流运行执行重试，对运行中任务执行取消。
...
13. 在移动端使用灵感产物列表、灵感产物工作台画布和连续生图页面的响应式操作入口。
