...
Inspiration One is an open-source, self-hosted inspiration creative workspace for solo merchants, small operations teams, and developers who want to manage AI creative workflows on their own infrastructure.

It is not a hosted SaaS inspiration, not a multi-tenant open platform, and does not promise to replace human operational judgment. The current core goal is:

> Move one inspiration from input assets to editable copy, downloadable posters, reusable image assets, and traceable workflow state.

## 2. Target Users

- Merchants who need to quickly create inspiration titles, selling points, main images, and promotional posters.
...
### 3.1 Inspiration Creative Chain

1. Log in with an admin key.
2. Create a inspiration, upload the inspiration source image, fill in the inspiration name, and choose a blank canvas or ecommerce scenario template.
3. Enter the inspiration workbench and add category, price, inspiration notes, and generation direction.
...
6. Download images/posters, or review inspiration asset history in the right-side Library panel.
7. On mobile, the inspiration list uses cards and floating pagination; inspiration detail keeps the canvas as the main view and opens workflow run, Single node, Templates, Details, Runs, and Library from the bottom toolbar.

### 3.2 Iterative Image Sessions

1. Create a standalone image session, optionally attached to a inspiration.
...
6. Attach satisfactory generated images back to a inspiration as reference assets, save them as inspiration main-image references, or collect them in the gallery.
7. On mobile, use a main-view, drawer, and bottom-sheet layout: the top bar exposes the session drawer, current session title/rename, and history drawer; the main view keeps status, current result, and provider notes; the left drawer manages sessions; the narrow right drawer selects branch/candidate history; the bottom generation sheet carries inspiration linking, references, prompt, size, candidate count, and advanced image tool parameters.

### 3.3 Inspiration DAG Workflow

1. Open the workflow workbench in the inspiration detail page.
2. Create or adjust nodes: inspiration context, reference image, copy generation, and image generation.
...
7. While running, the frontend polls lightweight workflow status; after completion, it refreshes full workflow, inspiration detail, and historical artifacts.
...
3. Gallery entries keep source session, linked inspiration, prompt, size, model, and download entrypoint.

### 3.5 In-Inspiration Help
...
3. Return from the help page to the inspiration workbench or Image chat.

## 4. Core Objects

- `Inspiration`: inspiration entity, including name, category, price, and input assets.
- `SourceAsset`: inspiration asset, including original main images, reference images, processed inspiration images, and other types.
- `CreativeBrief`: system-generated inspiration understanding result that provides shared semantics for copy and posters.
...
- `InspirationWorkflow` / `WorkflowNode` / `WorkflowEdge` / `WorkflowRun`: inspiration DAG workflow structure and run records.
...
- `/inspirations`: inspiration list.
- `/inspirations/new`: create inspiration.
- `/inspirations/:inspirationId`: inspiration detail, copy/poster main chain, history, and DAG workflow.
- `/gallery`: generated image gallery.
- `/help`: in-inspiration help page.
- `/settings`: provider, model, upload limit, job retry, and other business configuration.
- `/image-chat` and `/inspirations/:inspirationId/image-chat`: iterative image generation and attaching assets back to inspirations.

## 6. V1 Implemented Acceptance Surface

For a single self-hosted deployment, the current version should be able to:

1. Run the inspiration chain with local `mock` providers without external model keys.
2. Store inspirations, assets, copy, posters, tasks, image sessions, and workflow state in PostgreSQL.
3. Use Redis + Dramatiq to execute async copy/poster jobs and inspiration workflows.
...
5. Create inspirations with full scenario templates and insert the same built-in scenario templates or user-saved node-group templates in the workbench.
...
8. Retry recoverable iterative image tasks and inspiration workflow runs, and cancel running tasks.
...
12. Read in-inspiration operation guidance and support boundaries at `/help`.
13. Use responsive mobile entrypoints for the inspiration list, inspiration workbench canvas, and iterative image page.
