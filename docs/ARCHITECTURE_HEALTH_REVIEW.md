...
Inspiration One 当前架构处在“单商家自托管工作台已经可持续迭代”的状态。后端保持 FastAPI presentation /
application / domain / infrastructure 四层结构，灵感产物 DAG 工作流和连续生图 durable task 都以 PostgreSQL 状态为
权威，Redis/Dramatiq 只承担投递和后台执行。前端使用 React、TypeScript、TanStack Query，API client 和 DTO
集中在 `web/src/lib/`，灵感产物详情页已经开始按 page-local 组件和工具拆分。

相比历史审查，几项关键治理已经落地：

- InspirationWorkflow application 已拆成 graph、mutations、query、execution、context、artifacts、dependencies 等模块，
  `inspiration_workflows.py` 现在主要承担兼容 facade。
- 前端已有 ESLint 和 Vitest 脚本，不再是只有 TypeScript build 的状态。
- `InspirationDetailPage.tsx` 已把 Runs、Images、Inspector、NodeCard、canvas utils、download helpers 等拆到
  `web/src/pages/inspiration-detail/`。
...
- 连续生图和灵感产物工作流运行中都使用轻量 status polling，完成后再刷新完整详情。
...
后端代码位于 `backend/src/inspiration_one_backend/`：

- `presentation/`：FastAPI app、路由、schemas、鉴权依赖、上传校验和错误映射。
- `application/`：灵感产物、传统文案/海报任务、连续生图、画廊、生成准入和灵感产物工作流 use case。
- `domain/`：共享枚举和领域错误类型。
...
InspirationWorkflow application 当前已经不是单文件承载全部职责。当前模块包括：

- `inspiration_workflows.py`：对路由保持稳定的 facade。
- `inspiration_workflow/graph.py`：工作流查询、默认图结构和轻量 status snapshot。
- `inspiration_workflow/mutations.py`：节点、连线、参考图槽位和文案节点编辑。
- `inspiration_workflow/query.py`：灵感产物工作流详情查询。
- `inspiration_workflow/execution.py`：运行创建、节点调度和非图片节点执行。
- `inspiration_workflow/run_state.py`：节点运行 claim、失败、取消和 capacity requeue 状态推进。
- `inspiration_workflow/image_generation.py`：生图节点执行、provider timeout、安全失败和产物写回。
- `inspiration_workflow/context.py`：上游上下文收集和 provider 输入上下文构建。
- `inspiration_workflow/artifacts.py`：CopySet、SourceAsset、PosterVariant 等产物写回 helper。
- `inspiration_workflow/templates.py`：画布模板 materialization helper。
- `inspiration_workflow/user_templates.py`：用户保存节点组模板的 create/list/rename/archive/apply use case。
- `inspiration_workflow_dependencies.py`：执行依赖注入 seam，供测试和未来编排注入 text/image provider 与 renderer。

当前有两类后台执行状态：

- `WorkflowRun` / `WorkflowNodeRun`：灵感产物 DAG 工作流运行。
...
- `App.tsx`：当前路由入口，包含 `/login`、`/inspirations`、`/inspirations/new`、`/inspirations/:inspirationId`、
  `/image-chat`、`/inspirations/:inspirationId/image-chat`、`/gallery`、`/settings`。
- `pages/`：登录、灵感产物列表、创建灵感产物、灵感产物详情、连续生图、画廊、设置页。
- `pages/inspiration-detail/`：灵感产物详情 workbench 的 page-local 组件、canvas helpers、下载 helpers、测试和类型。
...
灵感产物 DAG 工作流已经从集中 application 文件拆成按职责命名的模块。路由仍通过稳定 facade 调用，降低了拆分对 API 层的影响。
...
- `ImageGalleryEntry` 保留来源会话、round、关联灵感产物、提示词、尺寸、模型和下载入口。
...
- 灵感产物工作流运行中轮询 `/api/inspirations/{inspiration_id}/workflow/status`。
...
这降低了灵感产物详情和连续生图页面在运行中反复序列化大对象、重渲染历史图像和覆盖本地交互状态的风险。

## 4. 当前主要风险

### R1. Application 仍承担较多领域规则

InspirationWorkflow 已拆分，但业务规则仍主要在 application 层直接围绕 SQLAlchemy models 编排。短期可接受；如果继续增加节点级 retry、跳过、复制、版本对比和 provider 分流，规则会继续分散在 execution、mutations、context 和 artifacts 之间。

建议保持渐进治理：只有当同一规则在多个 use case 中重复出现，或测试必须绕过大量数据库状态才能验证时，再抽 domain service/value object，不提前做“大领域模型重写”。

### R2. 前端组件级回归覆盖不足

Vitest 已经覆盖 helper 和部分 hook，但 InspirationDetail workbench 的真实交互仍依赖人工验证，包括节点拖拽、连接线创建/删除、右侧 panel 切换、保存草稿后运行、图片填充和下载误触发保护。
...
1. **优先补 InspirationDetail workbench 关键交互测试**
