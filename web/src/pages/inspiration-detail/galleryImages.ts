import type { PosterVariant, InspirationDetail, SourceAsset, InspirationWorkflow } from "../../lib/types";
...
export function buildPosterSourceAssetMap({
  ...
  for (const asset of inspiration.source_assets) {
...
export function getVisibleReferenceAssets({
  ...
  const postersById = new Map(posters.map((poster) => [poster.id, poster]));
  return [...inspiration.source_assets]
