...
import type { InspirationWorkflow, WorkflowEdge, WorkflowNode } from "../../lib/types";
import {
  INSPIRATION_ONE_EDGE_TYPE,
  INSPIRATION_ONE_NODE_TYPE,
  INSPIRATION_ONE_SOURCE_HANDLE,
  INSPIRATION_ONE_TARGET_HANDLE,
...
function makeEdge(partial: Partial<WorkflowEdge> & Pick<WorkflowEdge, "id">): WorkflowEdge {
  return {
    id: partial.id,
    workflow_id: partial.workflow_id ?? "workflow-1",
    source_node_id: partial.source_node_id ?? "inspiration",
...
function makeWorkflow(): InspirationWorkflow {
  return {
    id: "workflow-1",
    ...
    active: true,
    nodes: [
      makeNode({ id: "inspiration", node_type: "inspiration_context", position_x: 100.2, position_y: 200.4 }),
...
describe("reactFlowAdapters", () => {
  it("maps workflow nodes without changing backend snake_case fields", () => {
    ...
    expect(nodes[0]).toMatchObject({
      id: "inspiration",
      type: INSPIRATION_ONE_NODE_TYPE,
      position: { x: 160, y: 260 },
      selected: false,
      className: "nopan",
    });
    expect(nodes[0].data.workflowNode.node_type).toBe("inspiration_context");
  ...
  it("preserves ReactFlow internal drag positions over older optimistic drops while syncing node data", () => {
    const workflow = makeWorkflow();
    const nodes = workflowToReactFlowNodes(workflow, {
      selectedNodeIds: ["inspiration", "copy"],
      previousNodes: [
        { id: "inspiration", position: { x: 180.25, y: 260.75 } },
        { id: "copy", position: { x: 500, y: 320 } },
      ],
      preservePreviousPositionsForNodeIds: ["inspiration", "copy"],
      positionOverrides: {
        copy: { x: 520, y: 340 },
      },
    });
  ...
  it("maps workflow edges with stable handle fallbacks", () => {
    ...
    expect(edges[0]).toMatchObject({
      id: "edge-default",
      type: INSPIRATION_ONE_EDGE_TYPE,
      source: "inspiration",
      target: "copy",
      sourceHandle: INSPIRATION_ONE_SOURCE_HANDLE,
      targetHandle: INSPIRATION_ONE_TARGET_HANDLE,
    });
  ...
  it("computes deterministic centered lane offsets for fan-out edges", () => {
    expect(
      buildWorkflowEdgeLaneOffsets([
        makeEdge({ id: "edge-copy", source_node_id: "inspiration", target_node_id: "copy" }),
        makeEdge({ id: "edge-image", source_node_id: "inspiration", target_node_id: "image" }),
  ...
  it("measures group node drag distance in screen pixels using the current zoom", () => {
    ...
    expect(
      getNodeDragGroupScreenDistance(
        [
          { nodeId: "inspiration", position: { x: 101, y: 201 } },
  ...
  it("uses the screen-pixel drag guard to decide whether a group drop should persist", () => {
    ...
    expect(
      shouldCommitNodeDragGroupPosition(
        [{ nodeId: "inspiration", position: { x: 102.9, y: 200 } }],
        startPositions,
        1,
        3,
      ),
    ...
    expect(
      shouldCommitNodeDragGroupPosition(
        [{ nodeId: "inspiration", position: { x: 103, y: 200 } }],
        startPositions,
        1,
        3,
      ),
    ...
    expect(
      shouldCommitNodeDragGroupPosition(
        [
          { nodeId: "inspiration", position: { x: 101, y: 200 } },
  ...
  it("builds workflow edge input from a ReactFlow connection without local edge materialization", () => {
    expect(
      ...
      source_handle: INSPIRATION_ONE_SOURCE_HANDLE,
      target_handle: INSPIRATION_ONE_TARGET_HANDLE,
    });
    expect(
      connectionToWorkflowEdgeInput({
        source: "source",
