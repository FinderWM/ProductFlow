...
export function workflowNodeStatusLabel(node: Pick<WorkflowNode, "node_type" | "status">, t: TranslateFunction = defaultT): string {
  if (node.node_type === "inspiration_context" && node.status === "idle") {
...
export function getWorkflowNodeCancelableRun(
  workflow: InspirationWorkflow | undefined | null,
...
export function hasActiveWorkflow(workflow: InspirationWorkflow | undefined | null): boolean {
  if (!workflow) {
    return false;
  }
  return (
...
export function isInspirationWorkflowStatusActive(status: InspirationWorkflowStatus | undefined | null): boolean {
  if (!status) {
    return false;
  }
  return (
...
export function mergeInspirationWorkflowStatusIntoDetail(
...
export function shouldRefreshInspirationWorkflowDetailFromStatus(
  workflow: InspirationWorkflow | undefined | null,
  status: InspirationWorkflowStatus,
): boolean {
  ...
  if (hasActiveWorkflow(workflow) && !isInspirationWorkflowStatusActive(status)) {
    return true;
  }
