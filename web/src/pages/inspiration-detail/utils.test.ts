...
import type { InspirationWorkflow, WorkflowNode, WorkflowRun, WorkflowRunStatusSummary } from "../../lib/types";
import {
...
function workflowWith(overrides: Partial<InspirationWorkflow>): InspirationWorkflow {
  return {
    id: "workflow-1",
...
describe("inspiration-detail utils", () => {
  ...
  it("labels idle inspiration context nodes as usable static context", () => {
    expect(workflowNodeStatusLabel({ ...baseNode, node_type: "inspiration_context", status: "idle" })).toBe("可用");
  ...
  it("merges lightweight workflow status without replacing structure or node outputs", () => {
    ...
    const merged = mergeInspirationWorkflowStatusIntoDetail(workflow, {
      id: "workflow-1",
      inspiration_id: "inspiration-1",
      title: "默认工作流",
      active: true,
  ...
  it("refreshes the full workflow when lightweight status reaches a terminal state", () => {
    ...
    expect(
      shouldRefreshInspirationWorkflowDetailFromStatus(activeWorkflow, {
        id: "workflow-1",
        inspiration_id: "inspiration-1",
        title: "默认工作流",
        active: true,
