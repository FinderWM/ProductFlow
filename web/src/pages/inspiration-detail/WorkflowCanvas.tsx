...
import {
  ...
  type InspirationOneEdgeData,
  type InspirationOneNodeData,
...
type WorkflowCanvasNode = Node<WorkflowCanvasNodeData, typeof INSPIRATION_ONE_NODE_TYPE>;
type WorkflowCanvasEdge = Edge<WorkflowCanvasEdgeData, typeof INSPIRATION_ONE_EDGE_TYPE> & {
...
function InspirationOneCanvasNode({ data, dragging, isConnectable }: NodeProps<WorkflowCanvasNode>) {
  ...
  return (
    <div data-workflow-node-id={node.id} className="nopan relative w-[272px]">
      ...
      <Handle
        type="target"
        id={INSPIRATION_ONE_TARGET_HANDLE}
      ...
      <Handle
        type="source"
        id={INSPIRATION_ONE_SOURCE_HANDLE}
...
function InspirationOneCanvasEdge({
  id,
  sourceX,
  sourceY,
  targetX,
...
const nodeTypes = {
  [INSPIRATION_ONE_NODE_TYPE]: memo(InspirationOneCanvasNode),
};

const edgeTypes = {
  [INSPIRATION_ONE_EDGE_TYPE]: memo(InspirationOneCanvasEdge),
};
...
export const WorkflowCanvas = forwardRef<WorkflowCanvasHandle, WorkflowCanvasProps>(function WorkflowCanvas(
  ...
  const handleNodeDragStart = useCallback<OnNodeDrag<WorkflowCanvasNode>>(
    (event, node, draggedNodes) => {
      ...
      // Keep parent selection/sidebar updates out of the drag-start frame; they can trigger heavy InspirationDetail renders.
