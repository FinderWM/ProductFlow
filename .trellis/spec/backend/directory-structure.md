...
│   │   ├── inspiration_workflows.py          # stable facade for route/worker inspiration workflow imports
│   │   ├── inspiration_workflow/             # inspiration workflow internals split by owner module
│   │   │   ├── artifacts.py              # workflow artifact materialization and summaries
│   │   │   ├── context.py                # inspiration/upstream/reference execution context helpers
...
│   │   └── use_cases.py                 # inspiration/copy/poster workflow use cases
│   ├── presentation/
...
└── tests/
    ...
    ├── test_inspiration_crud_jobs.py
    ├── test_inspiration_workflow_*.py
...
- Resource routes are grouped under `presentation/routes/`, for example:
  - `presentation/routes/inspirations.py` handles `/api/inspirations`, copy sets, posters, and source-asset downloads.
...
- Request/response models and serializer functions live in `presentation/schemas/`, for example
  `presentation/schemas/inspirations.py` and `presentation/schemas/image_sessions.py`.
...
application functions such as `create_inspiration(...)`, `submit_inspiration_workflow_run(...)`, or
...
- `application/image_sessions.py` owns continuous image-session orchestration, including session CRUD, durable generation
  task lifecycle, worker failure settlement, attaching generated assets back to inspirations, and deleting session storage.
...
- Inspiration workflow application logic is split by executable boundary:
  - `application/inspiration_workflows.py` is the stable facade for route/queue/worker imports. Keep existing public use-case
  ...
  - `application/inspiration_workflow/graph.py` owns inspiration workflow graph loading, default graph templates, lookup helpers,
    topological ordering, and latest-run ordering. Keep these graph/query concerns out of
    `application/inspiration_workflows.py`.
  - `application/inspiration_workflow/mutations.py` owns workflow graph/edit use cases: create/update/delete nodes and edges,
    upload/bind reference images, edit generated copy, normalize the inspiration-context singleton, and shared node patch
    semantics such as title/config normalization plus failed-node repair reset.
  - `application/inspiration_workflow/execution.py` owns workflow run kickoff/execution, selected-node planning, and node
  ...
  - `application/inspiration_workflow/run_state.py` owns workflow run/node-run state transitions: atomic node-run claiming,
    run failure/cancel marking, capacity-wait requeue scheduling, and safe workflow failure reasons.
  - `application/inspiration_workflow/image_generation.py` owns the `image_generation` node executor, including generated
    poster/reference artifact creation, provider failure sanitization, provider timeouts, and concurrent render/provider
    calls.
  - `application/inspiration_workflow/query.py` is the narrow workflow query-service trial for execution/reuse hot paths
  ...
  - `application/inspiration_workflow_dependencies.py` owns explicit workflow execution dependency seams for text/image
    ...
    `inspiration_workflows.py` facade.
  - `application/inspiration_workflow/context.py` owns inspiration/incoming context collection, config parsing, upstream text
    assembly, reference input collection, and downstream reference target discovery.
  - `application/inspiration_workflow/artifacts.py` owns workflow artifact summaries and materialization helpers such as
    workflow-local copy sets, reference slot fill, generated image records, and poster-to-reference source lookup.
  - `application/inspiration_workflow/templates.py` owns canvas template graph materialization helpers.
  - `application/inspiration_workflow/user_templates.py` owns user-saved canvas template create/list/rename/archive/apply
    use cases.
...
`application/image_generation_core.py` owns provider-agnostic image generation helpers that are shared by inspiration workflow
...
- Most inspiration/image-session/settings route handler names end with `_endpoint`, e.g. `create_inspiration_endpoint` and
  `generate_image_session_round_endpoint`; `presentation/routes/auth.py` keeps shorter names such as `create_session`.
- Internal helper functions are prefixed with `_`, e.g. `_raise_http_error`, `_get_inspiration_or_raise`,
...
- Pydantic response/request classes use descriptive `PascalCase` names ending in `Response` or `Request`, for example
  `InspirationDetailResponse`, `ConfigUpdateRequest`, and `GenerateImageSessionRoundRequest`.
- SQLAlchemy models use singular `PascalCase` class names and plural table names, e.g. `Inspiration` -> `inspirations`,
